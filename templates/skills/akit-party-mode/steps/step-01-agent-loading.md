...
- Agent manifest is available at `.agent/agents.yaml`
- User configuration from `.agent/config.yaml` is loaded and resolved
...
Load and parse the agent manifest from `.agent/agents.yaml`

### 2. Extract Agent Data

Parse YAML to extract complete agent information for each entry:
